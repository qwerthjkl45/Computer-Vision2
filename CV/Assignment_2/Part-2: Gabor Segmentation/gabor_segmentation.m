...
for jj = 1 : length(gaborFilterBank)
    ...
    real_out = imfilter(img_gray, filterpairs(:, :, 1));  % \\TODO: filter the grayscale input with real part of the Gabor
    imag_out = imfilter(img_gray, filterpairs(:, :, 2));  % \\TODO: filter the grayscale input with imaginary part of the Gabor
...
for jj = 1:length(featureMaps)
    ...
    val = ((real_part .^ 2) + (imag_part .^ 2)) ;
    featureMags{jj} =(val);% \\TODO: Compute the magnitude here
...
features = standardization(features); % \\ TODO: i)  Implement standardization on matrix called features. 
...
function features_out = standardization(features_in)

    features_out = zeros(size(features_in));
    for num_feature = 1:size(features_in, 1)
        m = mean(features_in(num_feature, :));
        d = std(features_in(num_feature, :));
        
        features_out(num_feature, :) = (features_in(num_feature, :) - m) / d;
    end
end
